...
function insertSpez(e, prg) {
	...
	var selText = e.getSelection().getSelectedText();
	if (selText == "") {
	...
	if (!found && (prg!=12)) {
		alert("Es muss etwas markiert werden!");
	} else {
		if (prg == 12) { //ort nach paar und zurück))
			...
			if (found==2) { //image selected
				...
				var teile = get_teile(selText.trim());
				...
				if (teile.length > 1) {
					...
					if (prg == 1) {
						...
						htm = "<input type='text' size='" + zahl_str.length + "' data-baa-input='" + zahl_str + "' value='" + zahl_str + "' placeholder='?'>" + erg;
					} else if (prg == 2) {
						htm = "<span class='eAntw zuDrop' style='width:auto' data-baa-lsg='0'>" + selText.trim() + "</span>";
					}
				}
	
			}
		}
		e.insertHtml(vor + htm + nach);
	}
}
